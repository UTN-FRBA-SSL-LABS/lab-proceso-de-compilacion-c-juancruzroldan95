...
> **R:** Porque el archivo `.i` tiene el código C expandido. Esta es la etapa 1 del proceso de compilación. Este archivo tiene código C, esto significa que todavía no está compilado. Se podría decir que es una transpilación. El preprocesador incluye todas las directivas en este archivo `.i`.
...
> **R:** Esto ocurre porque en los `.i` no hay comentarios. El preprocesador elimina los comentarios. Los comentarios no aportan nada al proceso de compilación. A fines del "bundle" que se genera para compilar, los comentarios no sirven de nada.
...
> **R:** Estas lineas indican el archivo original de cada bloque de código. La declaración de `printf()` proviene del archivo "stdio.h", es el header estándar de C.
...
> **R:**
> ```asm
>     pushq   %rbp              ; Reserva/guarda espacio en la pila
>     movq    %rsp, %rbp        ; Establece el frame pointer
>     movl    %ecx, 16(%rbp)    ; Guarda parámetro `a` en la pila
>     movl    %edx, 24(%rbp)    ; Guarda parámetro `b` en la pila
> ```

Explicá en términos generales qué hacen esas instrucciones
(usá los comentarios del laboratorio como guía):

> **R:** `pushq %rbp` guarda el base pointer anterior en la pila (para poder restaurarlo al salir de la función). `movq %rsp, %rbp` establece el nuevo frame pointer, fijando la base del marco de pila de esta función. `movl %ecx, 16(%rbp)` y `movl %edx, 24(%rbp)` guardan los parámetros `a` y `b` en la pila — en la convención de llamada de Windows x64, el primer argumento llega en `%ecx` y el segundo en `%edx`. Es el equivalente x86-64 de lo que en ARM64 hacen `str w0, [sp, #12]` y `str w1, [sp, #8]`.
...
> **R:** En `programa.o` tiene el tipo U porque no está definido en ese archivo, pero si se utiliza en ese archivo.
> Por lo tanto, `programa.o` sabe que existe y que lo necesita, pero lo tiene que ir a buscar a otro lado.
> Ese otro lado es `matematica.o`, acá si está definido `area_circulo`, y por eso aquí tiene el tipo T.
...
> **R:** Quedan porque son funciones que están en bibliotecas dinamicas. Éstas se resuelven en tiempo de ejecución, no de compilación.
...
> **R:** La macro `CUADRADO(x)` es una sustitución de texto pura: el preprocesador la expande antes de que el compilador vea el código. `CUADRADO(5)` se convierte en `((5) * (5))` en el archivo `.i`. Desaparece en la etapa de **preprocesamiento**, no genera ningún símbolo en la tabla de símbolos ni aparece en el `.s`.
>
> La función real `sumar(a, b)` es código C que el compilador traduce a instrucciones en el `.s`, genera un símbolo `T` en el `.o`, y su referencia queda pendiente hasta el **enlazado** (si es externa).
>
> La diferencia clave: **solo la función real tiene verificación de tipos**. El compilador puede comprobar que `sumar(3, 4)` recibe exactamente dos `int`. Con una macro el compilador nunca ve `CUADRADO` — ya fue expandida — así que no puede detectar un `CUADRADO("hola")` como error de tipo.
...
> **R:** `T` (Text): el símbolo vive en la sección `.text`, que contiene **código ejecutable**. Corresponde a funciones (`sumar`, `main`, `imprimir_separador`). Son instrucciones de máquina.
>
> `D` (Data): el símbolo vive en la sección `.data`, que contiene **variables globales con valor inicial explícito** (ej: `int x = 5;`). El valor se guarda en el archivo objeto porque el programa lo necesita al arrancar.
>
> En este proyecto `llamadas` apareció como `B` (no `D`), porque es `int llamadas` sin inicializador — vive en `.bss`, que el SO pone en cero sin necesidad de guardarlo en el archivo.
...
> **R:** En Windows con MinGW no existe `ldd` ni `otool`. El equivalente es `objdump -p programa | grep "DLL Name"`, cuya salida es:
> ```
> DLL Name: msvcrt.dll
...
> **R:** `msvcrt.dll` es la biblioteca de C runtime de Windows (equivalente a `libc` en Linux). `gcc` la enlaza automáticamente porque está configurado para incluirla en cada compilación por defecto — igual que en Linux incluye `glibc` sin que se lo pidas. El programador no tiene que escribir `-lc` porque el driver de `gcc` lo agrega implícitamente siempre.
